...
pub fn start() -> (Ini, u8) {
    ...
    (configuration, args.debug_level)
}
...
fn read_startup_configuration(
    ...
    let mut config_file_location = 
        utilities::get_path_from_array(default_location);
    if !cli_location.as_deref().unwrap_or("").is_empty() {
        config_file_location = 
            cli_location.as_deref().unwrap_or("").to_string();
    }
