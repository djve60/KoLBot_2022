...
// Need to "use crate::global_constants".
// Refrenced with "global_constants::<NAME>".
...
pub const CHAT_EVENT_MESSAGE_RE: &str =
    r#"\{"type":"(event)","msg":"(.+?)who=(\d+).+?>([^<]+)<.+link":"?([^"]+)"?,"time":"(\d+)"#;
pub const CHAT_PRIVATE_MESSAGE_RE: &str =
    r#"\{"type":"(private)","msg":"(.+?)".+name":"(.+?)","id":"(\d+)"#;
    //r#"\{"type":"(private)","msg":"(.+?)"#;
    //r#"\{"type":"(private)"#;
pub const CHAT_PUBLIC_MESSAGE_RE: &str =
...
// Commands can match a minimum substring, from the start.
// This variable keeps them in the one place for easy reference.
lazy_static! {
    pub static ref COMMAND_SUBSTRINGS: HashMap<&'static str, &'static str> = {
        let mut m = HashMap::new();
        m.insert("fortune",  "fo");
        m.insert("rules",    "ru");
        m
