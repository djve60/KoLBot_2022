...
pub fn login(configuration: &Ini, kol_strings: &Ini, debug_level: u8) -> (bool, reqwest::Client) {
    let mut retry: bool = true;
    let mut login_status: bool = false;

    let mut client = initialize_client(constants::EMPTY_STR);

    while retry {
        let (mut game_status, mut landing_page_data) =
            check_game_status(&mut client, configuration, kol_strings, debug_level);

        // Sleep until the daily maintenance period is finished.
        while game_status.eq(&constants::KOL_MAINT_STR.to_string()) {
            let sleep_time = get_sleep_length();
        ...
        while game_status.eq(&constants::KOL_DOWN_STR.to_string()) {
        ...
        // Login!
        if game_status.eq(&constants::KOL_OPEN_STR.to_string()) {
...
pub fn logout(
    ...
    let (logout_page_data, _) = get_kol_data(client, uri, raw_data, debug_level);

    parse_kol_data::parse_logout_page(kol_strings, logout_page_data.body, debug_level)
}
...
pub async fn get_kol_data(
    ...
    let response_body = response.text().await.expect("Failed to get the HTML body.");
...
fn check_game_status(
    ...
    //let (landing_page_data, client) = get_kol_data(client, uri, raw_data, debug_level);
    ...
    let game_status =
        parse_kol_data::parse_landing_page(kol_strings, &landing_page_data.body, debug_level);
...
fn do_login(
    ...
    //let (login_page_data, client) = get_kol_data(&mut client, uri, raw_data, debug_level);
    let (login_page_data, _) = get_kol_data(&mut client, uri, raw_data, debug_level);

    //println!("LPD:\n{}", login_page_data.body);

    (
        parse_kol_data::parse_login_page(kol_strings, login_page_data.body, debug_level),
        client,
    )
}
...
fn get_sleep_length() -> u16 {
    ...
    if constants::GENERAL_MAINT > game_minutes {
        if game_datetime.weekday().to_string() != "Sun" {
            sleep_length = (constants::GENERAL_MAINT - game_minutes) * 60;
        } else {
            sleep_length = (constants::SATURDAY_MAINT - game_minutes) * 60;
        }
    } else {
